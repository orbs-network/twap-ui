...
//     const filterCondition = limit ? (order: ParsedOrder) => order?.totalChunks === 1 : (order: ParsedOrder) => (order?.totalChunks || 0) > 1;
//     const _orders = _.filter(_.flatMap(data), (it: ParsedOrder) => it?.status !== Status.Expired);
//     return _.groupBy(_.filter(_orders, filterCondition), (order: ParsedOrder) => order?.status);
//   }, [dataUpdatedAt, limit]);

//   const selectedOrder = useMemo(() => {
//     return _.find(_.flatMap(orders), (order: ParsedOrder) => order?.id === selectedOrderID) as ParsedOrder | undefined;
...
//       flatOrders[0] && setSelectedOrderID(flatOrders[0]?.id);
...
//   const amount = hooks.useFormatNumber({ value: orderUI?.srcAmountUi });
...
//           <Components.Base.TokenLogo logo={orderUI?.srcToken?.logoUrl} />
...
//   return <OrderDetail label="Progress" value={<Typography>{`${orderUI?.progress}%`}</Typography>} />;
// };
...
//           <Components.Base.TokenLogo logo={orderUI?.dstToken?.logoUrl} />
//         </Styles.StyledRowFlex>
//       }
//     />
//   );
...
//   const amount = hooks.useFormatNumber({ value: orderUI?.dstMinAmountOutUsdUi });
//   const tooltip = hooks.useFormatNumber({ value: orderUI?.dstMinAmountOutUsdUi, decimalScale: 18 });
//   const { limit } = useOrdersContext();

//   if (limit || orderUI?.isMarketOrder) return null;
//   return (
//     <OrderDetail
...
//   const { translations } = useTwapContext();
//   const amount = hooks.useFormatNumber({ value: orderUI?.srcChunkAmountUi });
//   const tooltip = hooks.useFormatNumber({ value: orderUI?.srcChunkAmountUi, decimalScale: 18 });
//   const { limit } = useOrdersContext();
...
//   return <OrderDetail label="Status" value={getStatusName(orderUI?.status)} />;
// };
...
//   const amount = hooks.useFormatNumber({ value: orderUI?.totalChunks });
...
//       value={<Typography>{fillDelayText(orderUI!.fillDelay, translations)}</Typography>}
//     />
//   );
// };
...
//   if (limit) return null;
//   return <OrderDetail labelTooltip={translations.maxDurationTooltip} label={translations.deadline} value={<Typography>{orderUI?.deadlineUi}</Typography>} />;
// };

// const CancelOrder = ({ className = "", orderUI }: { className?: string; orderUI?: OrderUI }) => {
//   if (orderUI?.status !== Status.Open) return null;
//   return <StyledCancelButton className={className} orderId={orderUI!.id} />;
// };
...
//             {orderUI.srcToken?.symbol}/{orderUI.dstToken?.symbol}
//           </StyledOrderSymbolsTop>
//           <StyledOrderSymbolsBottom theme={theme}> Buy {orderUI.dstToken?.symbol}</StyledOrderSymbolsBottom>
//         </StyledOrderSymbols>
//       </StyledOrderHeader>
...
//       <Components.Base.TokenLogo className="src" logo={orderUI?.srcToken?.logoUrl} />
//       <Components.Base.TokenLogo className="dst" logo={orderUI?.dstToken?.logoUrl} />
//     </StyledPairLogos>
//   );
// };
...
//           return <DesktopListItem key={o.id} order={o} />;
//         })
//       )}
//     </StyledList>
//   );
...
//       Buy {orderUI?.dstToken?.symbol} with {orderUI?.srcToken?.symbol}{" "}
//     </StyledBuyText>
//   );
// };
...
//   const srcAmount = hooks.useFormatNumber({ value: orderUI?.srcAmountUi, decimalScale });

//   return <StyledAmounts className={className}></StyledAmounts>;
// };
...
//   return (
//     <StyledListItem theme={theme} selected={orderUI?.order.id === selectedOrderID ? 1 : 0} onClick={() => orderUI && setSelectedOrderID(orderUI?.id)}>
//       <BuyText orderUI={orderUI} />
//       <Styles.StyledColumnFlex style={{ width: "auto", alignItems: "flex-end", gap: 2 }}>
//         <Amounts orderUI={orderUI} />
//         <StyledListItemStatus>{getStatusName(order?.status)}</StyledListItemStatus>
//       </Styles.StyledColumnFlex>
//     </StyledListItem>
//   );
// };
