...
type Action = "cancel order" | "wrap" | "approve" | "create order" | "module-import" | "ui-crashed";
...
export class Analytics {
  ...
  onConfigChange(config: Config) {
    if (config.partner !== this.data?.partner) {
      this.data = {
        _id: this.data.action === "module-import" ? this.data._id : generateId(),
