...
import { getDestTokenMinAmountPerChunk } from "@orbs-network/twap-sdk";
...
export const useDstMinAmountPerChunk = () => {
  ...
  const amountWei = useMemo(
    () => getDestTokenMinAmountPerChunk(srcTokenChunkAmount, price, Boolean(isMarketOrder), srcToken?.decimals || 0),
    [srcTokenChunkAmount, price, isMarketOrder, srcToken?.decimals],
  );
