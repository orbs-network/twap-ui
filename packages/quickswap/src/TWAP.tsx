      ...
      <AdapterStyles.StyledTokenPanel>
        <Components.Card>
          <TwapStyles.StyledColumnFlex gap={16}>
            <TwapStyles.StyledRowFlex justifyContent="space-between">
              <Components.Label className="twap-panel-title">{isSrcToken ? translations.from : `${translations.to} (${translations.estimated})`}</Components.Label>
              {children}
            </TwapStyles.StyledRowFlex>
            <TwapStyles.StyledRowFlex justifyContent="space-between">
              <Components.Tooltip text={selectTokenWarning}>
                <button onClick={onOpen} className={`twap-token-select ${token ? "twap-token-select-selected" : ""}`}>
                  {token ? <TokenDisplay logo={token?.logoUrl} name={token?.symbol} /> : <AdapterStyles.Text>{translations.selectToken}</AdapterStyles.Text>}
                </button>
              </Components.Tooltip>
